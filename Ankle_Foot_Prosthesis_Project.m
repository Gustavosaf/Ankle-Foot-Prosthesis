%% Workspace (clear all); Figures (close all); Command Window (clc)
...
model = Model();
model.setName('Ankle_Foot_Prosthesis');

ground = model.getGround(); % Get a reference to the ground object.

model.setGravity(Vec3(0,-9.80665,0)); % Define the acceleration of gravity.
...
%% Joints and coordinates in the model.
...
Joint1 = PinJoint('JointPlatform',...         
                  ground,...                  
                  Vec3(0,GroundToBody1,0),... 
                  Vec3(0,0,0),...             
                  Body1,...                   
                  Vec3(0,0,0),...             
                  Vec3(0,0,0));                  
Body1_rz = Joint1.upd_coordinates(0);         
Body1_rz.setRange([deg2rad(-17), deg2rad(9.5)]); 
Body1_rz.setName('Platform_rz');              
Body1_rz.setDefaultValue(deg2rad(-1.8));      
Body1_rz.setDefaultSpeedValue(0);
Body1_rz.setDefaultLocked(false);
model.addJoint(Joint1);

Joint2 = PlanarJoint('JointFootUpper', ... 
                     ground, ...
                     Body3);
Body3_rz = Joint2.upd_coordinates(0); 
...
Body3_tx = Joint2.upd_coordinates(1); 
...
Body3_ty = Joint2.upd_coordinates(2); 
Body3_ty.setRange([0,1]);
Body3_ty.setName('Foot_Upper_ty');
Body3_ty.setDefaultValue(GroundToBody1 + Body2ToBody1 + Body1Height + Body2Height - 0.2);
...
Joint3 = PinJoint('JointAnkle', ...
                  ...
                  Vec3(0,0,0),...
                  Vec3(0,0,0));
Body2_rz = Joint3.upd_coordinates(0); 
...
%% Contantact geometry to the model. 

PlatformContactGeometry = ContactHalfSpace();
PlatformContactGeometry.setName('PlatformContact');
PlatformContactGeometry.setLocation( Vec3(0,0,0));
PlatformContactGeometry.setOrientation( Vec3(0,0,-deg2rad(90)) );
...
FootContactGeometry.setLocation( Vec3(0,0,0) );
FootContactGeometry.setOrientation( Vec3(0,0,0) );
FootContactGeometry.setFrame(Body2);
model.addContactGeometry(FootContactGeometry);

%% Contact forces to the model (Elastic Foundation Force).

EFF = ElasticFoundationForce();
EFF.setName('EFFFootPlatform');
...
%% Initialize the System | Save the model to a file | Print on command window.
model.initSystem();
model.print('Ankle_Foot_Prosthesis.osim'); 
disp('Ankle_Foot_Prosthesis.osim printed!');